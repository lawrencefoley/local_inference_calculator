...
.. code-block:: bash
   ...
   uv run local-inference-calculator --model 0.6 --context 8192
   uv run local-inference-calculator -m 3.8 -c 4096 -q int4
...
.. code-block:: bash
   ...
   uv run local-inference-calculator --model 34 --context 8192
...
.. code-block:: bash
   ...
   for q in fp32 fp16 int8 int4; do
       echo "=== $q ==="
       uv run local-inference-calculator -m 7 -c 8192 -q $q | grep "TOTAL VRAM"
   done
...
.. code-block:: bash
   ...
   uv run local-inference-calculator --model 13 --context 16384 -q int4 --gpu-type datacenter | grep -i colab
...
.. code-block:: bash
   ...
   uv run local-inference-calculator -c 8192 --export-csv results.csv
...
.. code-block:: bash
   ...
   uv run local-inference-calculator --model 7 --context 8192 --format fp16
   uv run local-inference-calculator --model 7 --context 8192 --format gguf
   uv run local-inference-calculator --model 7 --context 8192 --format exl2
